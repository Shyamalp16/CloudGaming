...
void initializeGlobalConfig() {
    ...
    const auto mmcssEnv = Environment::read("INPUT_THREAD_MMCSS_CLASS");
    if (mmcssEnv) {
    ...
    const auto priorityEnv = Environment::read("INPUT_THREAD_PRIORITY");
    if (priorityEnv) {
        try {
    ...
    const auto enableMmcssEnv = Environment::read("INPUT_THREAD_ENABLE_MMCSS");
    if (enableMmcssEnv) {
        globalPriorityConfig.enableMMCSS = (*enableMmcssEnv == "1" ||
                                           *enableMmcssEnv == "true" ||
                                           *enableMmcssEnv == "TRUE");
    ...
    const auto enableTimeCriticalEnv = Environment::read("INPUT_THREAD_ENABLE_TIME_CRITICAL");
    if (enableTimeCriticalEnv) {
        globalPriorityConfig.enableTimeCritical = (*enableTimeCriticalEnv == "1" ||
                                                  *enableTimeCriticalEnv == "true" ||
                                                  *enableTimeCriticalEnv == "TRUE");
    ...
    const auto taskNameEnv = Environment::read("INPUT_THREAD_TASK_NAME");
    if (taskNameEnv) {
