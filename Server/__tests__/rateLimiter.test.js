...
function createFakeRedis() {
  ...
  return {
    async eval(_script, { keys, arguments: args }) {
      ...
      store.set(key, {
        value: val,
        expireAt: store.get(key)?.expireAt || Date.now() + Number(args[0]) * 1000,
      });
      return val;
    },
...
describe('RateLimiter', () => {
  ...
  it('fails closed in production when Redis is unavailable', async () => {
    ...
    const limiter = RateLimiter(redis, 'test:', true);
    await expect(
      limiter.allow({ namespace: 'connection', id: 'client', limit: 10, periodSeconds: 10 }),
    ).resolves.toBe(false);
  });
  ...
  it('uses a fail-closed local fallback on redis error', async () => {
    const broken = {
      async eval() {
    ...
    const request = { namespace: 'ip', id: 'x', limit: 1, periodSeconds: 10 };
    expect(await rl.allow(request)).toBe(true);
    expect(await rl.allow(request)).toBe(false);
  });
});
